...
We're combining the ts table with the "metrics table we just created and reassigned "tsid" to the "tsunique" column.
```{r}
...
We are trying to find the specific stock used to generate the graphs in the published data. To do this, we filtered the data via stockid and areaid. 
```{r}
cod_2J3KL <- fish |>
  filter(scientificname == "Gadus morhua") |>
...
Once we determine the relevant stock in the specific area, we need to determine during which years the stock collapsed. In this case, collapsed is determined whether the total catch was less than 10% of the historical maximum. However, the historical maximum is contingent to the timespan of the data. 
```{r}
collapse<-cod_2J3KL |> 
...
The following code replicates what was done previously, but instead of filtering by area ID, we determined the correct stock through trial and error of the area names. From there, we added the most recent data table, which contained information about total catch amounts. We then plotted this data by year and total catch. It should be noted that the original graph from the Millenium Ecosystem Assessment contains data from 1850 to 2000. Since we filtered the data earlier to the more recent values, our graph starts in 1950. Data from before 1950 excluded because it is not considered as reliable.
```{r}
cod |>
  filter(scientificname == "Gadus morhua") |>
...
#collapse value if true if stock has dropped below 10% of historical maximum
all_species
```

The original study examined catch values from 1950 to 2003. We therefore had to assign this time span to a variable, time_span. From there, we were able to determine the number of species that had continuously appeared throughout this period. This was done by filtering the years to only those in the time span and counting the number of rows where the number of species appearances was equal to our specified time span. 
```{r}
time_span <- 1950:2003
...
After we established the number of species that would be present in this new data set (118), we could then calculate the collapse values for each species. 
```{r}
